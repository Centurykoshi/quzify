...
const LoadingQuestions = ({finished} : Props) => { 
    ...
    useEffect(()=> { 
        const interval = setInterval(() => { 
            ...
            let imageIndex = Math.floor(Math.random() * loadingImages.length);
            setCurrentImage(loadingImages[imageIndex]);
        }, 2000); 
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 w-[70vw] md:w-[60vw] flex flex-col items-center">
      <Image src={currentImage} width={400} height={400} alt="loading" />
